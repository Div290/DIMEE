    ...
    Examples::
        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                # if i == 11:
                #     thi_label.append(labels.tolist())
                #     thi_conf.append(conf.tolist())
                #     break
                # if i == 7:
                #     six_label.append(labels.tolist())
                #     six_conf.append(conf.tolist())
                # if i == 11:
                #     fin_label.append(labels.tolist())
                #     fin_conf.append(conf.tolist())

                if conf >= self.patience:
                    # embedding_norm = torch.norm(embedding, dim=-1)  # Compute norm of embedding
                    # embeddings.append(embedding_norm.tolist())
                    # thi_conf.append(conf.tolist())
                    exit_layer.append(i)
                    break
            # print(len(six_label), len(thi_label), len(fin_label))
            ...
            # embeddings.append(embedding_norm.tolist())
            # labels_lis.append(labels.tolist())
            
        # with open('output_m_n_cola.csv', mode='w', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(['layer_number'])
        #     for thi_lbl, thi_cnf, six_lbl, six_cnf, fin_lbl, fin_cnf in zip(thi_label, thi_conf, six_label, six_conf, fin_label, fin_conf):
        #         writer.writerow([thi_lbl, thi_cnf, six_lbl, six_cnf, fin_lbl, fin_cnf])
        with open('output_qnli_l.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['layer_number'])
